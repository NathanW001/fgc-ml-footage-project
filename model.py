...
class VideoTransformer(keras.Model): 
    def __init__(self):
        super().__init__()
        self.conv1 = keras.layers.Conv2D(4, (3,3), activation='relu')
    ...
    def call(self, inputs):
        ...
        x = tf.divide(tf.divide(x, tf.multiply(tf.ones_like(x), norm_matrix)), tf.constant([[1,1,1,1,1,1,1,1,1]]))
        x = tf.reshape(x,[-1,3,3])
        # print(x)
        return spatial_transformer_custom.spatial_transformer_network(inputs, x)
        # return spatial_transformer_custom.spatial_transformer_network(inputs, tf.constant([[1,0,0],[0,1,0],[0,0,1]]))
